...
import java.util.HashMap; /*  Modify by SH  */
import java.util.Map;
import java.util.concurrent.ConcurrentHashMap;
import java.util.concurrent.locks.ReentrantLock;  /*  Modify by SH  */
...
public class FileMgr {
	...
	/*  Modify by SH  */
	private HashMap<String, ReentrantLock> fileLockMap = new HashMap<String, ReentrantLock>();
	...
	void read(BlockId blk, IoBuffer buffer) {
		...
		ReentrantLock lock = fileLockMap.get(fileName);
		if(lock!=null) {
			lock.lock();
		} else {
			fileLockMap.put(fileName, new ReentrantLock());
			lock = fileLockMap.get(fileName);
			lock.lock();
		}
		try {
